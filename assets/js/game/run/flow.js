...
function showUpgrade(reason){
 ...
 <div class="modal-foot"><span>1 / 2 / 3 choose · R reroll</span><button class="modal-btn" data-action="reroll" ${state.rerolls<=0?'disabled':''}>REROLL · ${state.rerolls}</button></div>`;
...
function die(){
 if(state.dead)return;state.dead=true;state.running=false;sfx('death');persistent.totalRuns++;const bank=Math.floor(state.scrap/45)+Math.max(1,state.sector-1);persistent.alloy+=bank;persistent.best=Math.max(persistent.best,state.sector);persistent.totalSalvage+=state.scrap;save();checkAchievements();
