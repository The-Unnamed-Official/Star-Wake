...
function pollGamepad(dt){
 ...
 const adOpen=typeof rerollAdActive!=='undefined'&&rerollAdActive;
 const wipeOpen=typeof wipeExperience!=='undefined'&&wipeExperience&&!wipeExperience.hidden;
 const modal=adOpen||wipeOpen||!frontLayer.classList.contains('hidden')||overlay.style.display==='grid'||drawer.classList.contains('open');
 if(!modal&&state?.running){input.gamepadX=ax;if(Math.abs(ax)>.35)state.lastDashDir=Math.sign(ax)}else input.gamepadX=0;
 ...
 if(modal&&pressed(0)&&!adOpen){const bs=interactiveButtons();bs[focusIndex]?.click()}
 if(modal&&pressed(1)){if(adOpen||wipeOpen)return;if(frontScreen!=='main'&&!frontLayer.classList.contains('hidden'))showMain();else if(drawer.classList.contains('open'))closeDrawer();else if(state?.paused)pause()}
 if(!modal&&state?.running){if(pressed(0))phaseShift();if(pressed(1))useBomb();if(pressed(9))pause();if(pressed(4)||pressed(5))toggleBuildMenu()}
